...
class TrajOptLQR():
    ...
    def __init__(self, agent):
        self.config = agent.traj_opt

        self.cons_per_step = self.config['cons_per_step']
        self._use_prev_distr = self.config['use_prev_distr']
        self._update_in_bwd_pass = self.config['update_in_bwd_pass']
    ...
    def update(self, m, algorithm):
        ...
        if not self.cons_per_step:
            min_eta = self.config['min_eta']
            max_eta = self.config['max_eta']
            LOGGER.debug("Running DGD for trajectory %d, eta: %f", m, eta)
        else:
            min_eta = np.ones(T) * self.config['min_eta']
            max_eta = np.ones(T) * self.config['max_eta']
    ...
    def backward(self, prev_traj_distr, traj_info, eta, algorithm, m):
        ...
        while fail:
            ...
            for t in range(T - 1, -1, -1):
                ...
                if self.config['update_in_bwd_pass']:
                ...
                if (self.cons_per_step or
                    not self.config['update_in_bwd_pass']):
            ...
            if not self.config['update_in_bwd_pass']:
